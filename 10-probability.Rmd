...
:::{.eg}
PDFs can be hard to understand at first, so here's an easy example to start. According to the [2011-12 CDC NHANES study](https://wwwn.cdc.gov/Nchs/Nhanes/2011-2012/BMX_G.htm), US adult human height has an approximately normal distribution with mean 65.8 inches and standard deviation 3.98 inches. Below is a plot of the distribution:
...
Remember the **density is NOT the probability**, for example the PDF is approximately $0.1$ around $x=66$. This does NOT mean there's a 10% chance of observing exactly of events correspond to areas under the curve. For example, one can show that for a normal distribution:
...
:::{.fold .s}
```{r,fig.height=3.8,tidy=F}
# use geom_function to plot dunif (the uniform PDF function)
ggplot() + geom_function(fun = dunif, xlim = c(-.5, 1.5), n = 1001) +
