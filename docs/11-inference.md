...
tibble(flips = rbinom(N*n, 1, 0.5),
       run = rep(1:20,each=10) %>% str_pad(2,pad="0") %>% paste0("run",.)) %>% 
  group_by(run) %>%
...
If instead of estimating a parameter, your primary goal is to test if a certain value for it agrees with the data, then you probably want to perform a **hypothesis test**.
...
In summary, here's the steps involved for a typical hypothesis test:
 ...
 3. If your p-value $<\alpha$, reject $H_0$ in favor of $H_a$. Otherwise, stay with $H_0$.
...
Next, we assume the null to be true and compute our p-value. Note that under the null assumption, we can model the number of heads $X\sim\bin(10,p=0.5)$. For reasons which will be explained soon, our "likeliness of data" for the p-value is actually defined as $P(X\geq7)$. We can find this using `pbinom()`.
