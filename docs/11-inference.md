...
tibble(flips = rbinom(N*n, 1, 0.5),
  ...
  theme(axis.text.y = element_text(family = "mono")) + labs(
    y = NULL, title = "Repeated 95% confidence intervals for p vs true probability p=0.5")
```

<img src="11-inference_files/figure-html/unnamed-chunk-9-1.svg" width="672" style="display: block; margin: auto;" />
:::

Remember that in the real-world, you don't know what the true probability of heads is, and you only have a single experiment's dataset, so you don't know if your interval captures the true value or not, but you can be 95% confident that it does.
