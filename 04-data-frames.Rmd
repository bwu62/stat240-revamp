...
df <- tibble(
  ...
  sex = c("F", "M", "M"),
  birthday = mdy(c("{format(today()-years(21),"%m/%d")%>%stringr::str_replace_all("(^|/)0","\\\\1")}/{format(today()-years(21),"%y")}", "7/4/99", "10/31/06")),
...
df <- tibble(
  name = c("Alice", "Bob", "Charlie"),
  sex = c("F", "M", "M"),
  birthday = c(today() - years(21), mdy(c("7/4/99", "10/31/06"))),
  age = floor(as.numeric(today() - birthday) / 365.24),
  declared_major = c(TRUE, TRUE, FALSE),
  school = "UW-Madison"
)
