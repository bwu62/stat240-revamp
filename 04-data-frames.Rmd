...
One additional non-text format covered in this course: XLS(X) spreadsheet data, commonly generated by Excel or similar spreadsheet software. For these, we have a different function from Tidyverse's non-core [readxl](https://readxl.tidyverse.org) package:

 - [`read_excel()`{.R}](https://rdrr.io/cran/readxl/man/read_excel.html) is used to read in both XLS and XLSX spreadsheet data.

:::{.note}
See the underscores in the function names? E.g. `read_csv()` which is from readr is NOT the same as `read.csv()` which is a base R function. They are similar, but readr's `read_csv()` has some minor improvements in speed and consistency so is recommended by this class.

Also note that if you do not have readr (or tidyverse) loaded, attempting to [TAB]{.k} autocomplete the `read_csv()` function will instead give you `read.csv()` so again, remember to set your working directory and load necessary libraries whenever (re)opening Rstudio and before starting/resuming your work.
:::
...
```{embed embed-eruptions-csv,echo=1:6,file="data/eruptions_recent.csv"}
```
...
Several things to note here:
 ...
 - Some diagnostic messages will be printed while reading, as well as any errors/warnings if it encounters anything unusual (no errors/warnings are observed here).
...
```{embed embed-eruptions-tsv,echo=1:6,file="data/eruptions_recent.tsv"}
```
...
```{embed embed-eruptions-delim,echo=1:6,file="data/eruptions_recent.delim"}
```
...
The `read_excel()` function from readxl also has some useful extra arguments. Some are the same as above, some are similar but slightly different, and some are unique to it (again, see help page). Brief selection of the most important arguments:
