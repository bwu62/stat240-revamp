...
T
```
```{r true-error1,eval=F}
true
```
```{r true-error2,echo=F,message=T}
...
```{r invalid-names1,tidy=F,eval=F}
# these variable names will raise errors:
#   1var, _var, bad-var,   e.g.:
1var <- 1
```
```{r invalid-names2,echo=F,eval=T,message=T}
...
```{r floating-point-errors,tidy=F}
1/2 + 1/3 == 5/6
1/2 + 1/3 - 5/6
sqrt(2)^2 == 2
sqrt(2)^2 - 2
...
```{r floating-point-equality-functions,tidy=F}
...
options(width = 70)
```
```{r load-tidyverse,message=T,class.message="wrapcode"}
library(tidyverse)
```
...
```
```{r reknit-demo,include=F,cache=T}
...
If you wish to read more on $\LaTeX$, start with Rong Zhuang's [MathJax cheat sheet](https://jojozhuang.github.io/tutorial/mathjax-cheat-sheet-for-mathematical-notation) or David Richeson's [quick guide](https://divisbyzero.github.io/latex-cheatsheet/latexcheatsheet.pdf) which both have lots of great beginner-friendly examples. For a slightly more complete list of symbols, Eric Torrence's [cheat sheet](https://pages.uoregon.edu/torrence/391/labs/LaTeX-cheat-sheet.pdf) may also be useful. 
