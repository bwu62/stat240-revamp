...
true
```
```{r true-error-2,echo=F,message=T}
message("Error: object 'true' not found")
```
...
As a last topic, let's briefly discuss packages. One of the best features of R is the ability for anyone to easily write and distribute packages on [CRAN](https://cran.r-project.org/) (**C**omprehensive **R** **A**rchive **N**etwork). Currently, there are `r rvest::read_html("https://cran.r-project.org/web/packages/") %>% rvest::html_text() %>% stringr::str_match("features \\d+ available packages") %>% stringr::str_extract("\\d+")` packages available on CRAN. There are also a further `r rvest::read_html("https://www.bioconductor.org/") %>% rvest::html_text() %>% stringr::str_match("Discover \\d+ software packages") %>% stringr::str_extract("\\d+")` packages on the bioinformatics-specific package archive [Bioconductor](https://www.bioconductor.org/), as well as countless more on [GitHub](https://github.com/topics/r-package).
...
```{embed embed.demo, file="demo.Rmd", class.source="wrapCode"}
```
```{r reknit demo.Rmd,include=F,cache=T}
rmarkdown::render("demo.Rmd", "html_document")
```
...
> ```{embed demo.yaml, file="demo.Rmd", echo=1:6, class.source="wrapCode"}
> ```
...
median(data)` ``
will render as: `r median(data)`. Note the `` `r `` prefix here, which triggers evaluation and substitute of the code. This helps avoid "hard-coding", letting values and references update to always stay in sync.


:::{.tip}
If a value has too many digits after the decimal, e.g. `` `r
mean(data)` ``
becomes: `r mean(data)`, it is ***highly recommended to round the result*** off to a reasonable number of digits using either `round()` or `signif()`. So in this case, `` `r
round(mean(data),2)` ``
which becomes `r round(mean(data),2)` is much better.
...
> ```{embed demo.chunk, file="demo.Rmd", echo=31:35, class.source="wrapCode"}
> ```
...
A code chunk has this basic structure:
 ...
 3. This name can also optionally be followed by a comma `,` followed by additional "chunk options", which are extra settings you can set to control the behavior of the chunk and output. There is a VERY long [list of available options](https://yihui.org/knitr/options/), but here is a short list of some of the MOST important:
    ...
    > ```{embed demo.setup, file="demo.Rmd", echo=8:13, class.source="wrapCode"}
    > ```
...
You can see my current Rstudio session (while writing these notes) is running from the `` `r basename(getwd())` `` directory which is itself located in `` `r dirname(getwd())` ``.
...
You can do this by either of these methods:
 ...
 - You can also set it automatically to your current Rmd file location by running `try(setwd(dirname(rstudioapi::getSourceEditorContext()$path)),silent=T)`.
