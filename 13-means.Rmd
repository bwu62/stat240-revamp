...
The Welch method models uses a t-based method to model the sample difference in means. The main equation of interest here is the degre of freedom equation, known as the Satterthwaite approximation:

$$
\nu=\frac{\left(s_1^2/n_1+s_2^2/n_2\right)^2}{\frac{(s_1^2/n_1)^2}{n_1-1}+\frac{(s_2^2/n_2)^2}{n_2-1}}
$$

This gives the approximate degrees of freedom that are relevant for both the Welch-based interval and t-test. Note this result is generally not an integer, which is perfectly fine to use in R, however if you ever need to round it to an integer (e.g. to use with a table), it must always be rounded *down* not up.

With this in hand, the confidence interval equation is:
...
- $t_{\alpha/2,\,\nu}$ is the **$\alpha$-level critical value for a t-distribution with $\nu$ degrees of freedom**,

- and $\se(\bar x_1-\bar x_2)=\sqrt{s_1^2/n_1+s_2^2/n_2}$ is the standard error for the difference in sample means.

In a similar fashion, for the Welch t-test, we test the following hypothese:
...
The test statistic is compputed as:
