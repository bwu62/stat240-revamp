...
Now let's look at a means-type inteference setup, where we apply a **normal model** to sample means to infer about an underlying **true mean** in the population. We will divide this into two scenarios again:

- One-mean scenario, where there is a single population with a single mean of interest, and

- Two-means scenario, where there are two populations, each with its own mean of interest, which we seek to compare.
...
tdist <- tibble(
  ...
  x = rep(seq(-xl,xl,length.out=N),length(unique(df))),
    t = dt(x,df),
    norm = dnorm(x)) %>% pivot_longer(t:norm, names_to = "dist", values_to = "y")
plot_ly(tdist, type = "scatter", x = ~x, y = ~y, frame = ~df, color = ~dist, mode = "lines") %>%
  layout(title = list(text = "T-distribution with various degrees of freedom vs normal", x = .05),
         margin = list(l = 10, r = 10, b = 50, t = 50)) %>% animation_opts(frame = 100) %>%
...
A few key facts that hopefully are intuitive after examining the plot:
 ...
 - Conceptually, t-distributions are basically a normal distribution except modified to have "heavier tails"
