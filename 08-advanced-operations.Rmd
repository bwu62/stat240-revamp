...
```{r}
penguins.bound <- bind_rows(penguins2007, penguins2008, penguins2009)
penguins.bound
```

It may be hard to see, since only the first 10 rows are printed here, but if you look at the row numbers, you can see we now have `r nrow(penguins2007)`+`r nrow(penguins2008)`+`r nrow(penguins2009)`=`r nrow(penguins.bound)` rows after binding.
