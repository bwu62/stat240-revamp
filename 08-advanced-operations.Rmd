...
Also, note the existence of several NAs in region and income. Again, verify this yourself, but these are all further subregion groupings of countries (e.g. AFE and AFW are for Eastern/Southern and Western/Central African countries), so we can simply drop these rows.
...
```{r}
fertility <- inner_join(fertility_meta2, fertility_raw2)
fertility
```
...
However, you may have noticed we still have a problem: our annual fertility rates are spread out across `r length(str_subset(names(fertility),"^\\d+$"))` columns, with each year in its own column. This is not compatible with our tidyverse functions. How do we fix it?
