...
fertility %>% 
  count(at_replacement = rate >= 2.1) %>% 
  mutate(pct = 100 * n / sum(n))
# we can repeat this grouping by region, this time
# just showing the percentage of countries at replacement
fertility %>% 
...
<center>
:::{.fold .s}
```{r,fig.width=7.2,fig.height=4.45}
library(plotly)
plot_ly(fertility, type = "choropleth", locations = ~code, z = ~rate,
        text = ~country, frame = ~year, zmin = 0.7, zmax = 8.9,
        colorscale = list(
          list(0, "rgb(255,0,0)"),
          list(0.1707, "rgb(255,255,255)"),
          list(1, "rgb(0,0,255)"))) %>% 
  layout(margin = list(l = 0, r = 0, b = 10, t = 31)) %>% 
