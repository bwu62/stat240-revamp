...
```'))
```
```{r date-example2,echo=F}
date <- ymd(paste(.y,"0704"))
date
```
...
```
```{r .dmyDemos2,echo=F,results='asis'}
cat(paste0('``` r
dmy(c(
  ',"\"",paste(.dmyDemos,collapse="\", \""),"\"",'
))
```'))
```
```{r .dmyDemos3,echo=F}
...
Let's continue using the generated `dates2` object above, except I will add our example date earlier `` `r date` `` into the vector as the first element.

```{r getters1,echo=F,results='asis'}
cat(glue::glue('``` r
# add in {.md}, then print (to remind us what it contains)
...
cat(glue::glue('``` r
# make a sequence of dates from {.md} to the end of the month
...
cat(glue::glue('``` r
# make a sequence of every {as.character(wday(date,label=T,abbr=F))} from {.md} to the end of the year
...
```
```{r date-inequality,echo=F}
# has {.md} passed yet? use today() to get today\'s date
#   (or technically, the last time these notes were compiled)
date <= today()
```
```{r date-min-sort-in}
