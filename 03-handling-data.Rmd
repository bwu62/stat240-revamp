...
Also note despite ``"`r date`"`` appearing to be a character, it is NOT a character. Using the `identical()` (which compares if two objects are the same) will show this to be false. Furthermore, `as.numeric()` confirms `date` converts to `r .dnum` as expected, whereas ``"`r date`"`` cannot be converted and returns `NA`.
```{r}
date
is.character(date)
```
