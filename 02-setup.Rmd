...
We will first install the latest release of R, which is version `r r.version` released on `r format(r.date,"%b %d, %Y")`; then we will install Rstudio. The instructions below are separated by operating system, depending on if you have a [Windows](#setup-win) or [Mac](#setup-mac) machine.
...
### Windows instructions {#setup-win}
 ...
 1. Download [R-`r r.version`-win.exe](`r str_glue("https://cloud.r-project.org/bin/windows/base/old/{r.version}/R-{r.version}-win.exe")`) and run it, accepting all default settings.
...
### Mac instructions {#setup-mac}
 ...
 1. First, we need to determine the right R installer file for your specific machine. Open the Apple menu in the top left corner of your screen and open ["About this mac"](https://upload.wikimedia.org/wikipedia/en/a/a6/Apple_menu_screenshot.png).
    - If it shows "Chip: Apple M1/M2/M3", download [R-`r r.version`-arm64.pkg](`r str_glue("https://cloud.r-project.org/bin/macosx/big-sur-arm64/base/R-{r.version}-arm64.pkg")`) and run it, accepting all default settings.
    - If it shows "Processor: ...Intel Core", download [R-`r r.version`-x86_64.pkg](`r str_glue("https://cloud.r-project.org/bin/macosx/big-sur-x86_64/base/R-{r.version}-x86_64.pkg")`) and run it, accepting all default settings.
...
::: {.rmdnote}
Note: before continuing, make sure that you can [open Rstudio](https://i.imgur.com/qmt5IHj.png) and have the correct latest version (R-`r r.version`) installed!
:::
