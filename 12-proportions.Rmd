...
Thus, a 95% confidence interval for $p$, i.e. the true probability of 4 or 5, is `r ci(c <- phat + c(-1,1) * qnorm(0.975) * sqrt(phat*(1-phat)/n))`, or in other words, **we are 95% confident the true $p$ is between `r round(c[1],2)` and `r round(c[2],2)`**.
...
We saw in our sample $\hat p=`r phat`$ was quite close to what we expected under a fair die of $0.4$. Let's formally test this using a hypothesis test.
...
Now we ask the key question for hypothesis testing: **what is the distribution of $X$ under the null hypothesis?**. Well under the null, we know $X\sim\bin(`r n`,0.4)$. This has the following distribution:
