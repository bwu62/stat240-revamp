...
```{r setup,include=F}
knitr::opts_chunk$set(tidy=F,eval=F)
```
...
```{r,echo=F}
volcanoes <- read_csv("data/volcanoes.csv")
volcanoes_raw <- read_csv("data/volcanoes_raw.csv")
```
...
```{r,echo=F}
eruptions_recent2 <- read_csv("data/eruptions_recent2.csv")
```
...
I also briefly needed a nice time series dataset with more than 1 groups to demonstrate line plots. Eventually I settled on [table 303.10](`r stringr::str_glue("https://nces.ed.gov/programs/digest/d{last.yy}/tables/dt{last.yy}_303.10.asp")`) of the National Center for Education Statistics (NCES) which contains historic college enrollment data, stratified by sex.
...
For an additional regression topic, I needed a good power-law dataset, and I found this excellent [paper by McNab (2008)](https://doi.org/10.1016/j.cbpa.2008.05.008) collecting the base metabolic rates of 638 species of mammals, which is known to follow a power law.
...
```{r}
bmr <- read_csv("data/icecream.csv", comment="#")
bmr
```
