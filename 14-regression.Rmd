...
There's a lot here to unpack, some of which we'll get to later, but for now we can see the results match our manual computation, and we have $\hat\beta_0=`r sf(B[1],4)`$, $\hat\beta_1=`r tr(B[2],6)`$.
...
We can use these to compute the standard errors for our example:

```{r}
icecream %>% summarize(
  sigma_hat = sqrt(1/(n()-2) * sum(residuals^2)),
  se_b0 = sigma_hat * sqrt(1/n() + mean(temperature)^2 / (sd(temperature)^2*(n()-1))),
  se_b1 = sigma_hat * sqrt(1 / (sd(temperature)^2*(n()-1)))
)
```
...
```
```{r,include=F}
SE <- sqrt(diag(vcov(lm_icecream)))
```

 - Under the `Std. Error` column, we can see $\se(\hat\beta_0)=`r sf(SE[1],4)`$, $\se(\hat\beta_1)=`r sf(SE[2],4)`$.
 
 - Below that, next to `Residual standard error`, we can see $\hat\sigma=`r sf(summary(lm_icecream)[["sigma"]],4)`$, which is our estimate of $\sigma$, i.e. the standard deviation of the $\ep_i$ errors.
...
$$
t_\obs~=~\frac{\hat\beta_1-0}{\se(\hat\beta_1)}~=~\frac{`r sf(B[2],4)`}{`r sf(SE[2],4)`}~=~`r sf(B[2]/SE[2],4)`~\sim~T_{n-2}
$$

```{r,echo=F,results='asis'}
cat(glue::glue('``` r
# since t_obs is positive, take twice the upper tail are
2 * (1 - pt({sf(summary(lm_icecream)$coefficients[2,3],4)}, df={summary(lm_icecream)$df[2]}))
```'))
```
```{r,echo=F}
2 * (1 - pt(signif(summary(lm_icecream)$coefficients[2,3],4), df=summary(lm_icecream)$df[2]))
```

These all match the summary, and we can conclude there's very strong evidence to suggest a significant linear relationship exists. We can also compute a 95% confidence interval for $\hat\beta_1$
...
```{r,echo=F,results='asis'}
cat(glue::glue('``` r
# get fitted slope and SE from summary output
{sf(B[2],5)} + c(-1,1) * qt(0.975, df={summary(lm_icecream)$df[2]}) * {sf(SE[2],4)}
```'))
```
```{r,echo=F}
signif(B[2],5) + c(-1,1) * qt(0.975, df=summary(lm_icecream)$df[2]) * signif(SE[2],4)
```
...
Again, everything agrees and we are satisfied. Our 95% confidence interval for the true slope $\beta_1$ is $`r ci(confint(lm_icecream, level = 0.95)[2,])`$.
