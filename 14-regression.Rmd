...
lm1 <- lm(consumption ~ temperature, data = icecream)
lm1
# print a more complete summary output of the model
summary(lm1)
```

There's a lot of info here to unpack, some of which we'll get to later and some of which are beyond our scope, but for now we can see the results match our manual computation, and we have $\hat\beta_1=`r coef(lm1)[2]`$, $\hat\beta_0=`r coef(lm1)[1]`$.
...
icecream$residuals
residuals(lm1) %>% unname  # remove names from vector, which are added by default
# plot residuals (note: since residuals=response-predicted, predicted=response-residuals)
ggplot(icecream, aes(x=temperature, y=consumption)) +
  geom_segment(aes(yend=consumption-residuals), color="orange") + geom_point() +
  geom_abline(slope=b1, intercept=b0, color="red") + labs(y="Consumption (pints per capita)",
  x="Mean temperature (F)", title="Ice cream consumption vs temperature (with residuals)")
```
